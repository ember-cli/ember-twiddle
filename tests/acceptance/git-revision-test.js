...
module('Acceptance | gist-revision', function(hooks) {
  ...
  test('Able to copy a revision into new gist', async function(assert) {
    ...
    await click('[data-test-output-menu]');
    await click("#live-reload");
    await click('.test-sidenav-toggle');
    await click('[title="Duplicate Twiddle"]');
    ...
    assert.dom('[title="Duplicate Twiddle"]').doesNotExist("Menu item to copy gist is not shown anymore");
